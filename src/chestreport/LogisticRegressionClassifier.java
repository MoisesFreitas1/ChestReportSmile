...
//		ResizeImages(dirTestPneumonia, dirTestResizedString, "pneumonia");
		...
		double lambda = 0.1;
		LogisticRegression logit = LogisticRegression.fit(inputTrain, expectedValueTrain, lambda, 1E-5, 500);
		...
		if (dirTestResized.isDirectory()) { // make sure it's a directory
            ...
            int[] pred = logit.predict(inputTest);
            for(int i = 0; i < pred.length; i++) {
            	if(pred[i] == expectedValueTest[i]) {
            		accuracy += 1;
            	}
            	if(pred[i] == 0 && pred[i] == expectedValueTest[i]) {
